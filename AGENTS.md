...
- **一局合并结算（无流局、无对局记录）**：开局后在本局面板录牌况——「🐴中马情况」选马（`state.currentHand.horses`）、「＋添加杠」进 `state.currentHand.kongs`（不立即改分、不占局数）、「＋添加胡牌」可**多条胡牌记录**（`state.currentHand.wins`，支持一炮多响：同一放炮者被多人吃胡/抢杠）；点「结算本局」`settleHand()` 把 马＋杠＋胡牌 合并成一笔结算（`round.wins` + `round.kongs` + `round.horses` + `round.changes`），一局=一条记录。对局页不显示历史列表，📊统计表顶部是各家总结算、`renderHandDetails()` 渲染每局明细。谁胡谁坐庄取第一条赢家。
